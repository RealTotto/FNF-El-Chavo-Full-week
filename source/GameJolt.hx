        ...
        if(userLogin)
        {
            GJApi.addTrophy(trophyID, function(data:Map<String,String>){
                ...
                if (data.exists("message"))
                    bool = true;
                Main.gjToastManager.createToast('GameJolt/$title', title.toUpperCase(), desc.toUpperCase(), true, color);
            });
        }
    }
...
{    
    ...
    */
    public static var fontPath:String = "assets/fonts/GROBOLD.ttf";
    /**
    ...
    */
    public static var imagePath:String = "GameJolt/LoginGJ";
...
class GameJoltLogin extends MusicBeatState
{
    ...
    override function create()
    {
        ...
        {
            if (!GameJoltAPI.getStatus())
                openLink('https://youtu.be/2zuRsBTa7-g');
            else
            {
