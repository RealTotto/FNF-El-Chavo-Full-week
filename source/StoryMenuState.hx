...
class StoryMenuState extends MusicBeatState
{
	...
	var leftArrowRar:FlxSprite;
	var rightArrowRar:FlxSprite;
	...
	override function create()
	{
		...
		leftArrowRar = new FlxSprite(FlxG.width * 0.05).loadGraphic(Paths.image('arrow_left'));
		leftArrowRar.screenCenter(Y);
		leftArrowRar.alpha = 1;
		leftArrowRar.antialiasing = ClientPrefs.globalAntialiasing;
		leftArrowRar.y -= 70;
		add(leftArrowRar);

		rightArrowRar = new FlxSprite(FlxG.width * 0.87).loadGraphic(Paths.image('arrow_left'));
		rightArrowRar.screenCenter(Y);
		rightArrowRar.alpha = 1;
		rightArrowRar.antialiasing = ClientPrefs.globalAntialiasing;
		rightArrowRar.angle = 180;
		rightArrowRar.y -= 70;
		add(rightArrowRar);
	...
	override function update(elapsed:Float)
	{
		...
		scoreText.text = "SEASON SCORE:" + lerpScore;

		// FlxG.watch.addQuick('font', scoreText.font);

		if (!movedBack && !selectedWeek)
		{
			var leftP = controls.UI_RIGHT_P;
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		{
			item.x = bullShit - curWeek;
			if (item.x == Std.int(0) && unlocked)
				item.alpha = 1;
			else
				item.alpha = 0;
			bullShit++;
