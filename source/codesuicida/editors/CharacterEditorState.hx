...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		textAnim.setFormat(Paths.font("GROBOLD.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var pcPortStage:BGSprite = new BGSprite('pcport/goal', 0, 0);
			pcPortStage.scale.set(1.3, 1.3);
			bgLayer.add(pcPortStage);
			pcPortStage.antialiasing = false;

			var widShit = Std.int(pcPortStage.width * 9);

			pcPortStage.setGraphicSize(widShit);

			pcPortStage.updateHitbox();
	...
	var TemplateCharacter:String = '{
			...
			"image": "characters/Retsuko_Assets",
			"position": [
				0,
				0
			],
