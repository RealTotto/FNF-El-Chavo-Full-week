...
class WeekEditorState extends MusicBeatState
{
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			...
			Assets.exists(Paths.getPath('images/VsSuicida/menubackgrounds/menu_' + assetName + '.png', IMAGE), IMAGE)) {
	...
	function reloadWeekThing() {
		...
		if(assetName != null && assetName.length > 0) {
			...
			Assets.exists(Paths.getPath('images/VsSuicida/storymenu/' + assetName + '.png', IMAGE), IMAGE)) {
		...
		if(isMissing) {
			...
			missingFileText.text = 'MISSING FILE: images/VsSuicida/storymenu/' + assetName + '.png';
		}
