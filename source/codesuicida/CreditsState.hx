...
class CreditsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		data = Json.parse(Paths.getTextFromFile('images/VsSuicida/credits/Data.json'));

		for (i in 0...data.participant.length)
		{

			var icon:FlxSprite = new FlxSprite().loadGraphic(Paths.image('VsSuicida/credits/icons/'+ data.participant[i][0]));
		...
		boxText = new FlxSprite(744, 535).loadGraphic(Paths.image('VsSuicida/credits/selectors/text_box'));
		...
		frameSelector = new FlxSprite().loadGraphic(Paths.image('VsSuicida/credits/selectors/select'));
	...
	function select() {
		...
		bg.loadGraphic(Paths.image('VsSuicida/credits/roles_images/' + data.roles[data.participant[curSelected][4]][0]));
		bg.setGraphicSize(1280,720);
		bg.updateHitbox();

		participants.loadGraphic(Paths.image('VsSuicida/credits/icons/' + data.participant[curSelected][0]));
