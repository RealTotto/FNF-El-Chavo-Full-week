...
class CategoryModsState extends MusicBeatState
{
	...
	var leText:AlphabetVs;
	private var grpTexts:FlxTypedGroup<AlphabetVs>;
	...
	{	
		...
		grpTexts = new FlxTypedGroup<AlphabetVs>();
		add(grpTexts);		

		for (i in 0...mods.length)
		{
			var leText:AlphabetVs = new AlphabetVs(0, (70 * i) + 30, mods[i], true, false);
	...
	override function update(elapsed:Float)
	{
		if (!selectedSomethin)
        {
			...
			if(controls.ACCEPT)
			{
				...
				{
					case 'El Chavo':
						MusicBeatState.switchState(new MainTitle());
