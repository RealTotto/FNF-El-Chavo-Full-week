...
class TitleCoversState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('CoversDelOcho/freakyMenu'), 0);
			}
		}
		...
		logoBl.frames = Paths.getSparrowAtlas('CoversDelOcho/logoBumpin');
		...
		titleBG = new FlxSprite().loadGraphic(Paths.image('CoversDelOcho/TitleBG'));
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					...
					//FlxG.sound.music.stop();
					createCoolText(['Los Covers Del Ochp By'], 15);
				case 2:
					addMoreText('LuisPlayerXy', 15);
				case 3:
					deleteCoolText();
				...
				// credTextShit.text += '\nEl Chavo';
				case 12:
					addMoreText('Del Ocho'); // credTextShit.text += '\nFunkin';				
