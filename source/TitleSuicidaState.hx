...
class TitleSuicidaState extends MusicBeatState
{
	public static var initialized:Bool = false;

	var blackScreen:FlxSprite;
	var credGroup:FlxGroup;
	var credTextShit:AlphabetVs;
	...
	function startIntro()
	{
		...
		logoBl.frames = Paths.getSparrowAtlas('VsSuicida/LogoBumpin');
		...
		titlestatebg = new FlxBackdrop(Paths.image('VsSuicida/loading'), 0.2, 0, true, true);
		...
		var path = "mods/" + Paths.currentModDirectory + "/images/VsSuicida/titleEnter.png";
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "mods/images/VsSuicida/titleEnter.png";
		}
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "assets/images/VsSuicida/titleEnter.png";
		}
		//trace(path, FileSystem.exists(path));
		...
		credTextShit = new AlphabetVs(0, 0, "", true);
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('VsSuicida/newgrounds_logo'));
		add(ngSpr);
		ngSpr.visible = false;
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxTween.tween(logoBl, {x: -2000}, 2, {ease: FlxEase.expoInOut});
				FlxTween.tween(titlestatebg, {y: -1000}, 3, {ease: FlxEase.expoInOut});						

				FlxG.camera.flash(ClientPrefs.flashing ? FlxColor.WHITE : 0x4CFFFFFF, 1);
				FlxG.sound.play(Paths.sound('VsSuicida/confirmMenu'), 0.7);
	...
	function createCoolText(textArray:Array<String>, ?offset:Float = 0)
	{
		...
		{
			var money:AlphabetVs = new AlphabetVs(0, 0, textArray[i], true, false);
	...
	function addMoreText(text:String, ?offset:Float = 0)
	{
		if(textGroup != null && credGroup != null) {
			var coolText:AlphabetVs = new AlphabetVs(0, 0, text, true, false);
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					...
					//FlxG.sound.music.stop();
					createCoolText(['Vs El Chavo Suicida By'], 15);
				case 2:
					addMoreText('Christ', 15);
				case 3:
					deleteCoolText();
				...
				// credTextShit.text += '\nEl Chavo';
				case 12:
					addMoreText('Suicida'); // credTextShit.text += '\nFunkin';				
