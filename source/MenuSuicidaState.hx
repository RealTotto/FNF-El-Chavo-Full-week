...
class MenuSuicidaState extends MusicBeatState
{
	...
	override function create()
	{
        ...
        var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('VsSuicida/menuBG'));
        ...
        if(ClientPrefs.themedmainmenubg == true) {
            ...
            var themedBg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('VsSuicida/menuDesat'));
            ...
            if(hours > 18) {
                ...
                themedBg.loadGraphic(Paths.image('VsSuicida/menuBG'));
            }
        }
        ...
        magenta = new FlxSprite(-80).loadGraphic(Paths.image('VsSuicida/menuDesat'));
		...
		for (i in 0...optionShit.length)
		{
			...
			menuItem.frames = Paths.getSparrowAtlas('VsSuicida/mainmenu/menu_' + optionShit[i]);
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			if (controls.BACK)
			{
				...
				LoadingState.loadAndSwitchState(new TitleSuicidaState());
			}

			if(FlxG.keys.justPressed.CONTROL)
			{
				LoadingState.loadAndSwitchState(new codesuicida.CategoryModsState());			
			}			
							...
							*/
...

							{
								...
								switch (daChoice)
								{
									...
									#if MODS_ALLOWED
									case 'mods':
									    MainMenuState.backMenus = true;
										MusicBeatState.switchState(new ModsMenuState()); 
									#end
									#if ACHIEVEMENTS_ALLOWED
									case 'awards':
										MusicBeatState.switchState(new codesuicida.AchievementsMenuState());
									#end										
									case 'credits':
										MusicBeatState.switchState(new codesuicida.CreditsState());
									case 'options':
										LoadingState.loadAndSwitchState(new codesuicida.options.OptionsState());
								}
							});
						}
					});
