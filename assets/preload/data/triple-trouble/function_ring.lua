...
--[[function onCreatePost()
    ...
    if not isPixel then 
    makeAnimatedLuaSprite('strumring', 'extra_note/ham_cake', ringpos[1], ringpos[2]);
    else
...
function onUpdatePost(elapsed)
	for i = 0, getProperty('notes.length')-1 do
		if getPropertyFromGroup('notes', i, 'noteType') == 'RingNote' then
            ...
            setPropertyFromGroup('notes', i, 'noteData', 0) -- idk why
			...
			if not getPropertyFromGroup('notes', i, 'isSustainNote') and getPropertyFromGroup('notes', i, 'strumTime') - getPropertyFromClass('Conductor', 'safeZoneOffset') * 0.7 < getSongPosition() then
            ...
            if not getPropertyFromGroup('notes', i, 'isSustainNote') and getPropertyFromGroup('notes', i, 'strumTime') - getPropertyFromClass('Conductor', 'safeZoneOffset') * 0.2 < getSongPosition() then
				if botPlay then 
					HitRing = true
					objectPlayAnimation('strumring', 'glow', true)
                    updateOffsets()
            ...
            if getPropertyFromGroup('notes', i, 'isSustainNote') and getPropertyFromGroup('notes', i, 'strumTime') - getPropertyFromClass('Conductor', 'safeZoneOffset') * 0.2 < getSongPosition() then
				if keyPressed('space') and not getProperty('boyfriend.stunned') or botPlay then 
					HitRing = true
					objectPlayAnimation('strumring', 'glow', true)
                    updateOffsets()
					Hit(i, true);
    ...
    if not botPlay then
    if keyJustPressed('space') and ringmechanic and not HitRing then
    ...
    if keyReleased('space') then
        objectPlayAnimation('strumring', 'strum', true) -- oh, thats a decreprated function ummmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm im too lazy to chnage it to "playAnim()..."
        HitRing = false
        updateOffsets()
    end
end
...
function makeyStuff()
    ...
    if not downscroll then
        makeLuaSprite('counter', 'extra_note/Counter', 1120, 610);
        else
        makeLuaSprite('counter', 'extra_note/Counter', 1120, 10);
    end
